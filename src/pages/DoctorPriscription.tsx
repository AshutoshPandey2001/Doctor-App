...
const DoctorPriscription = ({navigation, route}: any) => {
    ...
    if (focus) {
      const getAllPatients = async () => {
        ...
        try {
          ...
          console.log('all patients Clinical history', temp_data);
    ...
    querySnapshot.forEach(async doc => {
      try {
        ...
        await doc.ref.update({
          ...
          prescription,
          timestamp: new Date(
            state.timestamp.seconds * 1000 +
              Math.floor(state.timestamp.nanoseconds / 1e6),
          ),
        });
        dispatch(setLoading(false));
        navigation.goBack();
  ...
  const cancelAdvice = (item: any) => {
    setPrescription(
      prescription.filter((medicine: any) => medicine.id !== item.id),
    );
  ...
  const renderItem = ({item}: {item: any}) => (
    <View
      style={[GlobalStyle.card, {width: Dimensions.get('window').width - 50}]}>
      ...
      <View style={GlobalStyle.middleSide}>
        ...
        <Text
          ...
          numberOfLines={1}
          ellipsizeMode="tail">
          {item.frequency.M} - {item.frequency.A} - {item.frequency.E} -{' '}
          {item.frequency.N}
        </Text>
        <Text
          style={GlobalStyle.textcolor}
  ...
  const renderHeader = (section: any, isActive: number) => (
    <View
      ...
      <Text style={{color: '#000', fontSize: 16}}>
        Consulting Date :-{formatDateDDMMYYY(section.consultingDate)}
      </Text>
      <View>
      ...
      <View
        ...
        <View
          ...
          <Text
            ...
            {routeName ? 'Prescription' : 'Clinical History'}
          </Text>
        </View>
      </View>
              ...
              <View>
                ...
                <View style={styles.section}>
                  ...
                  <TextInput
                    ...
                    onChangeText={text =>
                      setAdvice({...advice, medicine: text})
                    }
                    placeholderTextColor={'gray'}
                  />
                ...
                <View style={styles.section}>
                  ...
                  <TextInput
                    ...
                    numberOfLines={3}
                    onChangeText={text =>
                      setState({...state, generalInstruction: text})
                    }
                    placeholderTextColor={'gray'}
                  />
                </View>
              ...
              <View>
                ...
                {history.length > 0 ? (
                  ...
                  <Text
                    ...
                    There are no clinical history to display
                  </Text>
                )}
              </View>
