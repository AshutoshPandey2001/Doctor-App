...
const Home = () => {
  ...
  const parentDocRefpatients = firestore()
    .collection('opdPatients')
    .doc('m5JHl3l4zhaBCa8Vihcb');
  const subcollectionRefpatients =
    parentDocRefpatients.collection('opdPatient');
  const onClose = () => {
    setisVisible(false);
  };
    ...
    switch (item) {
      case 'today':
        query = subcollectionRefpatients
          ...
          .where('consultingDate', '==', moment().format('YYYY-MM-DD[Z]'));
      ...
      case '7days':
        ...
        query = subcollectionRefpatients
          ...
          .where(
            'consultingDate',
            '<=',
            moment(formattedDate).format('YYYY-MM-DD[Z]'),
          )
          .where(
            'consultingDate',
            '>=',
            moment(lastSevenDays).format('YYYY-MM-DD[Z]'),
          );
        patientsData = await retrieveData(query);
        dayText = 'Last 7 days';
        break;
      case '30days':
        ...
        query = subcollectionRefpatients
          ...
          .where(
            'consultingDate',
            '>=',
            moment(lastThirtyDays).format('YYYY-MM-DD[Z]'),
          );
        patientsData = await retrieveData(query);
          ...
          <View style={styles.boxBody}>
            <View style={styles.flexContainer}>
              <View style={styles.imageContainer}>
                <Image
                  source={require('../assets/opd_icon.png')}
                  style={styles.image}
                  resizeMode="contain"
                />
              </View>
